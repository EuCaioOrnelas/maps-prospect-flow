    ...
    <div className="flex flex-col h-full overflow-hidden gap-2">
      <div className="grid grid-cols-[1fr_auto] gap-2 shrink-0">
        <div className="grid gap-1.5">
          ...
          <div className="bg-secondary rounded-lg px-3 py-1.5 text-[12px] text-muted-foreground flex items-center gap-1.5">
      ...
      <div className="rounded-xl bg-secondary/30 p-2 flex-1 min-h-0 flex flex-col gap-1.5 overflow-hidden">
          ...
          {leads.map((l, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-[13px] truncate">{l.name}</p>
                <p className="text-[12px] text-muted-foreground truncate">{l.phone} · {l.note}</p>
              </div>
              <div className="rounded-full bg-primary/10 px-2 py-0.5 text-[13px] font-semibold text-primary shrink-0">
                {l.rating}
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-2 gap-1.5 shrink-0">
          <div className="rounded-lg bg-success/10 p-1.5 border border-success/10">
            <p className="text-[13px] text-muted-foreground">Telefones validados</p>
            <p className="text-[12px] font-semibold text-success">3/3 com DDI + DDD</p>
          </div>
          <div className="rounded-lg bg-primary/10 p-1.5 border border-primary/10">
            <p className="text-[13px] text-muted-foreground">Prontos para CRM</p>
            <p className="text-[12px] font-semibold text-primary">Importação instantânea</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex h-full flex-col gap-2 overflow-hidden">
      <div className="flex items-center gap-2.5 bg-secondary/50 rounded-lg p-2 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[12px] text-muted-foreground">(11) 99XXX-XXXX</p>
        </div>
      ...
      <div className="rounded-xl bg-secondary/30 p-2 flex-1 min-h-0 flex flex-col gap-1.5 overflow-hidden">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="min-w-0">
            <p className="text-[12px] font-medium text-primary truncate">GPT cruzando sinais comerciais</p>
            <p className="text-[12px] text-muted-foreground truncate">Presença local, potencial e score preditivo</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1 shrink-0">
          {tags.map((t, i) => (
            <span
              key={i}
              className={`text-[13px] px-1.5 py-0.5 rounded-full font-medium ${t.color}`}
        ...
        >
          <p className="text-[12px] font-semibold text-primary">Diagnóstico automático</p>
    ...
    <div className="flex h-full flex-col gap-2.5">
      <div className="flex items-center gap-2 bg-secondary/50 rounded-lg p-2.5">
        ...
        <div>
          ...
          <p className="text-[12px] text-muted-foreground">João Silva — Proprietário</p>
        </div>
      </div>
      <div className="bg-secondary/30 rounded-lg p-3 flex-1 flex flex-col min-h-0">
        <div className="flex items-center gap-1.5 mb-2">
          <Sparkles size={11} className="text-primary animate-pulse" />
          <span className="text-[12px] font-medium text-primary">IA gerando mensagem personalizada</span>
        </div>
    ...
    <div className="flex h-full flex-col gap-2 overflow-hidden">
          ...
          return (
            <div key={i} className="bg-secondary/45 rounded-lg p-2 border border-border/40">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[13px] truncate">{c}</p>
                  <p className="text-[13px] text-muted-foreground truncate">Fila oficial · cadência inteligente</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  {delivered ? (
                    <span className="text-[13px] text-success font-medium flex items-center gap-0.5"><Check size={9} /> Entregue</span>
                  ) : sent ? (
                    <span className="text-[13px] text-primary font-medium flex items-center gap-0.5"><Clock size={9} className="animate-pulse" /> Enviando...</span>
                  ) : (
                    <span className="text-[13px] text-muted-foreground">Na fila</span>
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="bg-primary/10 rounded-lg p-2.5 border border-primary/10 shrink-0">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-[12px] font-medium text-primary">Disparo em massa</span>
          <span className="text-[12px] font-semibold text-primary ml-auto">{overallProgress}%</span>
        </div>
        <div className="w-full bg-primary/10 rounded-full h-1.5 overflow-hidden">
          <div className="bg-primary h-1.5 rounded-full transition-[width] duration-200" style={{ width: `${overallProgress}%` }} />
        </div>
        <div className="flex items-center justify-between gap-2 mt-1.5 text-[13px] text-muted-foreground">
  ...
  return (
    <div className="flex h-full flex-col gap-2.5">
            ...
            return (
              <div key={index} className={`flex ${message.dir === 'out' ? 'justify-end' : 'justify-start'}`} style={{ animation: 'fadeSlideUp 0.35s ease-out' }}>
                <div className={`${message.dir === 'out' ? 'bg-primary/15 rounded-tr-sm' : 'bg-background/70 rounded-tl-sm'} rounded-xl px-3 py-2 max-w-[84%] border border-border/30`}>
                  <p className="text-[13px] text-foreground leading-relaxed">{message.text}</p>
                  <div className="flex items-center justify-end gap-1 mt-1">
                    <span className="text-[12px] text-muted-foreground">{message.time}</span>
                    {message.dir === 'out' && (
                      <>
      ...
      <div className="bg-success/10 rounded-lg p-2.5 flex items-center gap-2 border border-success/10">
        ...
        <span className="text-[12px] font-medium text-success">Lead respondeu e abriu janela ativa para atendimento</span>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="flex h-full flex-col gap-2.5">
      <div className="flex items-center gap-2 rounded-xl bg-secondary/30 p-2.5 border border-border/40">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[12px] font-medium text-primary">Agente de IA respondendo em tempo real</p>
          <p className="text-[12px] text-muted-foreground truncate">Contexto, score, CRM e histórico da conversa</p>
        </div>
        <div className="rounded-full bg-primary/10 px-2 py-1 text-[12px] font-medium text-primary">GPT ativo</div>
      </div>
      <div className="rounded-xl bg-secondary/30 p-2.5 flex-1 min-h-0">
            ...
            return (
              <div key={i} className={`flex ${m.dir === 'out' ? 'justify-end' : 'justify-start'}`} style={{ animation: 'fadeSlideUp 0.35s ease-out' }}>
                <div className={`${m.dir === 'out' ? 'bg-primary/15 rounded-tr-sm' : 'bg-background/75 rounded-tl-sm'} rounded-xl px-2.5 py-1.5 max-w-[84%] border border-border/30`}>
                  <p className="text-[12px] text-foreground leading-relaxed">{m.text}</p>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-lg bg-success/10 p-2 border border-success/10">
          <p className="text-[13px] text-muted-foreground">Intenção detectada</p>
          <p className="text-[12px] font-semibold text-success">Alta prioridade comercial</p>
        </div>
        <div className="rounded-lg bg-primary/10 p-2 border border-primary/10">
          <p className="text-[13px] text-muted-foreground">Próximo passo</p>
          <p className="text-[12px] font-semibold text-primary">Reunião agendada</p>
        </div>
      </div>
    </div>
  );
...
const StageClose = ({ progress }: { progress: number }) => (
  <div className="flex h-full flex-col">
    <div className="flex items-center gap-2.5 bg-secondary/50 rounded-lg p-2.5">
      ...
      <div className="flex-1"><p className="font-medium text-xs">CrossFit Box SP</p><p className="text-[12px] text-muted-foreground">João Silva</p></div>
    </div>
    <div className="flex flex-col items-center justify-center gap-3 flex-1">
      ...
      {progress > 0.4 && (
        <div className="flex items-center gap-2 bg-info/10 rounded-full px-3 py-1.5" style={{ animation: 'fadeSlideUp 0.4s ease-out' }}>
          ...
          <span className="text-[13px] font-medium text-info">Reunião agendada — Sex 14:00</span>
        </div>
      )}
      {progress > 0.6 && (
        <div className="flex items-center gap-2 bg-warning/10 rounded-full px-3 py-1.5" style={{ animation: 'fadeSlideUp 0.4s ease-out' }}>
          ...
          <span className="text-[13px] font-medium text-warning">Valor: R$ 5.964/ano</span>
        </div>
      )}
      {progress > 0.8 && (
        <div className="flex items-center gap-2 bg-primary/10 rounded-full px-3 py-1.5" style={{ animation: 'fadeSlideUp 0.4s ease-out' }}>
          ...
          <span className="text-[13px] font-medium text-primary">Movido para CRM automaticamente</span>
        </div>
      )}
    </div>
  </div>
  ...
  return (
    <div className="flex h-full flex-col gap-2 overflow-hidden">
      <div className="relative flex-1 min-h-0 overflow-hidden">
          ...
          {columns.map((col, ci) => (
            ...
            >
              <div className="flex items-center gap-1 mb-1.5 shrink-0">
                ...
                <span className="text-[12px] font-semibold text-foreground truncate">{col.title}</span>
                <span className="text-[13px] text-muted-foreground ml-auto">{col.leads.length}</span>
                {col.showAdd && (
              ...
              <div className="space-y-1 flex-1 min-h-0 overflow-hidden">
                {col.leads.map((lead, li) => (
                  ...
                  >
                    <p className="font-medium text-[12px] text-foreground truncate">{lead.name}</p>
                    <div className="mt-0.5 flex items-center justify-between gap-1">
                      <span className={`text-[13px] font-bold ${col.accent}`}>{lead.score}</span>
                      <span className="text-[13px] text-muted-foreground/80 truncate">{lead.value}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="pointer-events-none absolute inset-0">
          ...
          {dragActive && (
            ...
            >
              <p className="font-medium text-[12px] text-foreground truncate">{createdLead.name}</p>
              <div className="mt-0.5 flex items-center justify-between gap-1">
                <span className="text-[13px] font-semibold text-primary">{createdLead.score}</span>
                <span className="text-[13px] text-muted-foreground">{createdLead.value}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-secondary/30 rounded-lg p-2 flex items-center gap-2 border border-border/40 shrink-0">
        ...
        <div className="min-w-0">
          <p className="text-[13px] font-medium text-foreground">Lead criado e movido automaticamente</p>
          <p className="text-[13px] text-muted-foreground truncate">Studio Pilates One → Qualificado · Score 684</p>
        </div>
      </div>
    </div>
  );
            ...
            <div className="relative w-full max-w-[28rem] 2xl:max-w-[30rem]">
                ...
                <div key={i} className={`absolute ${card.position} z-30 floating-card hidden lg:block`} style={{ animationDelay: card.delay }}>
                  <div className="glass rounded-lg p-3 shadow-lg shadow-primary/10 border border-border/50 hover:border-primary/20 transition-all hover:scale-105">
                    <div className="flex items-center gap-2">
                      ...
                      <div><p className="text-sm font-bold text-foreground"><AnimatedCounter value={card.value} duration={2000} /></p><p className="text-[12px] text-muted-foreground whitespace-nowrap">{card.label}</p></div>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div ref={demoRef} className={`relative glass rounded-xl sm:rounded-2xl p-4 sm:p-5 shadow-card hover:shadow-glow transition-shadow duration-500 ${isAnimating ? 'demo-animating' : 'demo-paused'}`}>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[13px] text-muted-foreground font-medium tracking-wide uppercase">Wiize Platform</span>
                </div>
                ...
                <div className="bg-background/50 rounded-lg sm:rounded-xl p-3 sm:p-4">
                  ...
                  <div className="flex items-center gap-2 mb-3 px-1">
                    ...
                    <span className="text-[13px] text-muted-foreground ml-auto">{currentStage + 1}/8</span>
                  </div>
                ...
                {/* Watermark */}
                <div className="absolute bottom-2 left-4 text-[12px] text-muted-foreground/40 font-medium tracking-wide">@wiizebrasil</div>
              </div>
            </div>
          </div>
        </div>
