...
export const HeroSection = ({ onSignupClick }: HeroSectionProps) => {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[85vh] flex items-center justify-center pt-16 pb-10 overflow-x-clip overflow-y-visible w-full">
      ...
      <div className="absolute inset-0 will-change-transform" style={{ transform: `translateY(${parallaxOffset * 0.5}px)`, background: "linear-gradient(180deg, hsl(158 40% 96%) 0%, hsl(210 30% 99%) 55%, hsl(var(--background)) 100%)" }} />
      {/* Dotted texture */}
      ...
      {/* Soft primary glow */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[1000px] md:w-[1200px] h-[1000px] md:h-[1200px] will-change-transform pointer-events-none" style={{ transform: `translate(-50%, ${parallaxOffset * 0.3}px)`, background: "radial-gradient(ellipse at center, hsl(158 72% 45% / 0.18) 0%, hsl(158 72% 45% / 0.06) 40%, transparent 70%)" }} />
      {/* Top-right accent blob */}
      <div className="absolute -top-32 -right-32 w-[600px] h-[600px] rounded-full pointer-events-none opacity-60" style={{ background: "radial-gradient(circle, hsl(158 72% 45% / 0.14) 0%, transparent 65%)" }} />
      {/* Bottom-left accent blob */}
      <div className="absolute -bottom-40 -left-32 w-[700px] h-[700px] rounded-full pointer-events-none opacity-50" style={{ background: "radial-gradient(circle, hsl(158 72% 45% / 0.10) 0%, transparent 65%)" }} />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.3fr_1fr] gap-2 xl:gap-4 items-center">
          ...
          <div className="text-center xl:text-left">
            ...
            <h1 className="font-display font-bold mb-4 sm:mb-6 animate-slide-up text-foreground leading-[1.08] tracking-tight" style={{ animationDelay: "0.1s" }}>
              ...
              <span className="block whitespace-nowrap text-shimmer-highlight font-extrabold text-[2rem] sm:text-[3.05rem] md:text-[3.85rem] lg:text-[4.1rem] xl:text-[4.5rem] leading-[1.02] mt-1 sm:mt-2">oportunidades reais</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center xl:justify-start gap-3 sm:gap-4 mb-8 animate-slide-up" style={{ animationDelay: "0.3s" }}>
              ...
              <button onClick={() => setVideoOpen(true)} className="group shrink-0">
                <Button
                  variant="ghost"
                  size="lg"
                  className="rounded-full text-sm sm:text-base px-6 sm:px-8 h-11 sm:h-12 border border-border/40 hover:border-foreground/20 hover:bg-secondary/40 transition-all duration-300"
                >
                  Ver Demonstração
                </Button>
              </button>
