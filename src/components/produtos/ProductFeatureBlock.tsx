...
function SplitTitle({ title, highlight }: { title: string; highlight?: string }) {
  ...
  return (
    <h2 className="mt-3 max-w-full font-display text-[clamp(1.55rem,3.05vw,2.6rem)] font-extrabold leading-[1.12] tracking-tight text-foreground">
      {parts.before && <span className={parts.match ? "block text-balance sm:whitespace-nowrap" : "line-clamp-2"}>{parts.before.trim()}</span>}
      {parts.match && (
        <span className="block text-balance text-shimmer-highlight sm:whitespace-nowrap">{parts.match}</span>
      )}
...
export const ProductFeatureBlock = ({ feature, visual, index = 0 }: ProductFeatureBlockProps) => {
  ...
  const visualColumn = (
    ...
    >
      <div
        className={`group mx-auto w-full min-w-0 max-w-full lg:mx-0 lg:max-w-[clamp(19rem,32vw,30rem)] ${reversed ? "lg:mr-auto" : "lg:ml-auto"}`}
      >
        <div className="overflow-hidden rounded-panel border border-border/60 bg-card/60">
          <div
            className={`relative h-[clamp(22rem,58vw,30rem)] overflow-hidden bg-gradient-to-br sm:h-[clamp(20rem,32vw,26rem)] lg:h-[clamp(19rem,26vw,24rem)] ${getCardTheme(visual)}`}
          >
            <div
            ...
            <div className="pointer-events-none absolute inset-x-0 top-7 flex justify-center px-4 lg:px-6">
              <div className="w-full max-w-full origin-top scale-100 transition-transform duration-500 lg:max-w-[26rem] lg:scale-[0.94] lg:group-hover:scale-[0.97]">
