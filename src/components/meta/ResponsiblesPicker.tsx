          ...
          <div className="max-h-56 overflow-y-auto overscroll-contain py-1">
            ...
            {filtered.map((m) => {
              ...
              return (
                ...
                >
                  <Avatar className="h-7 w-7 shrink-0 rounded-sm border border-border/60 bg-muted">
                    <AvatarImage className="rounded-sm object-cover" src={m.avatar_url || undefined} alt={label} />
                    <AvatarFallback className="text-[10px] rounded-sm bg-primary/10 text-primary font-medium">{initialsOf(label)}</AvatarFallback>
                  </Avatar>
        ...
        {value.length === 0 ? (
          ...
          value.map((id) => {
            ...
            return (
              ...
              >
                <Avatar className="h-5 w-5 rounded-xs border border-border/50 bg-muted">
                  <AvatarImage className="rounded-xs object-cover" src={m?.avatar_url || undefined} alt={label} />
                  <AvatarFallback className="rounded-xs bg-primary/10 text-[9px] text-primary font-medium">{initialsOf(label)}</AvatarFallback>
                </Avatar>
  ...
  return (
    <div className="flex items-center -space-x-2">
      {shown.map((id) => {
        ...
        return (
          <Avatar key={id} className="h-6 w-6 rounded-sm border-2 border-background bg-muted" title={label}>
            <AvatarImage className="rounded-sm object-cover" src={m?.avatar_url || undefined} alt={label} />
            <AvatarFallback className="text-[9px] rounded-sm bg-primary/10 text-primary font-medium">{initialsOf(label)}</AvatarFallback>
          </Avatar>
        );
      })}
      {userIds.length > max && (
          ...
          return (
            <Tooltip key={id}>
              <TooltipTrigger asChild>
                ...
                >
                  <Avatar className="h-7 w-7 rounded-sm border-2 border-background bg-muted shadow-sm ring-1 ring-border/40 transition-shadow duration-300 hover:shadow-md [&>img]:rounded-sm">
                    <AvatarImage className="rounded-sm object-cover" src={m?.avatar_url || undefined} alt={label} />
