...
export function MetaCustosPanel({ data }: MetaCustosPanelProps) {
  ...
  const [pricePerMsg, setPricePerMsg] = useState<number | "">(0.12);
  ...
  const kpis = [
    { label: "Custo Meta total", value: fmtBRLp(data.totalCost), raw: data.totalCost, delta: deltaPct(data.totalCost, data.prevTotalCost), accent: "primary" as const, icon: <DollarSign size={14} />, spark: data.sparks.cost, hint: `Custo total Meta no período. Real (recebido via webhook): ${fmtBRL(data.realCost)} · Estimado: ${fmtBRL(data.estimatedCost)} (${Math.round(data.costRealRatio * 100)}% real).` },
