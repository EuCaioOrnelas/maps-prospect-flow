...
const maturityLabel = (score: number, hasData: boolean) => {
  if (!hasData) return { text: "Dados insuficientes para cálculo", color: "text-muted-foreground" };
  if (score >= 80) return { text: "Excelente", color: "text-primary" };
  if (score >= 60) return { text: "Bom", color: "text-foreground" };
...
export const MaturityGauge = () => {
  ...
  const hasRealData = maturity.total > 0 || maturity.responsiveness > 0 || maturity.hotUtilization > 0;
  const label = maturityLabel(maturity.total, hasRealData);
      ...
      <CardContent className="space-y-4">
        {!hasRealData ? (
          <p className="text-sm text-muted-foreground/60 italic py-4 text-center">
            Aguardando atividade real para gerar índice.
          </p>
        ) : (
          <>
