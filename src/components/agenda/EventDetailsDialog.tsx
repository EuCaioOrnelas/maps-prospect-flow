...
export function EventDetailsDialog({
  ...
  const confirmMeta = confirm ? CONFIRM_COPY[confirm] : null;

  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v && !confirm) onOpenChange(false); }}>
      <DialogContent className="max-w-lg max-h-[92vh] overflow-y-auto" onPointerDownOutside={(e) => confirm && e.preventDefault()}>
        {confirm && confirmMeta ? (
          <div ref={confirmRef} className="flex flex-col items-center py-6 text-center">
            <div
              className={cn(
            ...
            >
              <ConfirmIcon className="h-8 w-8" />
            </div>

            <DialogHeader className="space-y-3">
              <DialogTitle className="text-xl">{confirmMeta.title}</DialogTitle>
              <DialogDescription className="max-w-[18rem] text-sm leading-relaxed">
