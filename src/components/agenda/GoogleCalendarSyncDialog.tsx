...
export function GoogleCalendarSyncDialog({ open, onOpenChange, onSynced }: Props) {
  ...
  const describe = (res: any) =>
    `${res.pushed || 0} enviados ao Google · ${res.updated || 0} atualizados`;
  ...
  const handleDisconnect = async () => {
    const ok = window.confirm(
      "Desconectar o Google Agenda? Os compromissos da Wiize continuam aqui e deixam de ser enviados para o Google.",
    );
    if (!ok) return;
    setSaving(true);
    try {
      ...
      setCalendars([]);
      setHealth("unknown");
      toast.success("Conexão removida. Os compromissos deixam de ser enviados ao Google.");
      onSynced?.();
      await loadStatus();
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <DialogDescription className="text-xs leading-relaxed">
                Os compromissos criados na Wiize são enviados automaticamente para o seu Google
                Agenda. Nada é importado do Google. Conexão individual por usuário.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
                  ...
                  {(syncing || progress > 0) && (
                    <div className="space-y-2 rounded-[14px] border border-primary/20 bg-primary/5 p-4">
                      ...
                      <p className="text-[11px] leading-relaxed text-muted-foreground">
                        Enviando os compromissos da Wiize para o Google Agenda…
                      </p>
                      {syncing && (
                        <Button
                          variant="outline"
