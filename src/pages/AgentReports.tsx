...
import { 
  ...
  AlertCircle, RefreshCw, Calendar, ChevronRight, BarChart3
} from "lucide-react";
import { format, subDays, parseISO, differenceInSeconds, differenceInMinutes, startOfDay } from "date-fns";
...
export default function AgentReports() {
    ...
    return agents.map(agent => {
      ...
      agentConversations.forEach(conv => {
        if (conv.response_received_at && conv.reply_sent_at) {
          ...
          if (responseTime > 0 && responseTime < 3600) { // Max 1 hour
      ...
      agentMessages.forEach(m => {
        if (m.content) {
          SENSITIVE_WORDS.forEach(word => {
            if (m.content?.toLowerCase().includes(word)) {
              sensitiveWordsCount++;
            }
          });
        }
      ...
      // Repeated messages detection (simplified - checking for exact duplicates)
      const messageContents = agentMessages
        .filter(m => m.direction === 'outgoing' && m.content)
        .map(m => m.content?.toLowerCase().trim());
      const uniqueMessages = new Set(messageContents);
      const repeatedMessagesRate = messageContents.length > 0 
        ? Math.round(((messageContents.length - uniqueMessages.size) / messageContents.length) * 100)
        : 0;
      ...
      // Calculate Health Score (5 pillars)
  ...
  const dailyMetrics = useMemo((): DailyMetrics[] => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      // Simplified health score for daily view
      let healthScore = 75;
      if (escalationRate > 30) healthScore -= 20;
      if (errorRate > 5) healthScore -= 15;
      if (conversationsStarted > 0 && messagesReceived / conversationsStarted < 1) healthScore -= 10;
  ...
  const statusDistribution = useMemo(() => {
    ...
    const other = filteredConversations.length - completed - escalated - awaiting;
  ...
  const funnelData = useMemo(() => {
    const total = aggregatedMetrics?.conversationsStarted || 0;
    const responded = conversations.filter(c => 
      c.response_received && 
      (selectedAgent === "all" || c.agent_id === selectedAgent)
    ).length;
    ...
    return [
      ...
      { name: 'Concluídas', value: completed, fill: '#8b5cf6' }
    ];
  }, [conversations, selectedAgent, aggregatedMetrics]);
  ...
  const insights = useMemo((): Insight[] => {
    ...
    if (aggregatedMetrics.avgResponseTime <= 5) {
      insightsList.push({
        type: 'success',
    ...
    return insightsList.slice(0, 5); // Max 5 insights
  }, [aggregatedMetrics]);
  ...
  const exportToExcel = () => {
    const exportData = agentMetrics.map(m => ({
      ...
      'Status': getHealthClassification(m.healthScore).label
    }));
  ...
  // Export to CSV
  const exportToCSV = () => {
    const headers = ['Agente', 'Conversas', 'Score', 'Encerramento %', 'Escalonamento %', 'Erros %', 'Repetidas %', 'Status'];
  ...
  const healthClass = aggregatedMetrics ? getHealthClassification(aggregatedMetrics.healthScore) : null;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="mt-3">
                    <p className="text-2xl font-bold">{aggregatedMetrics?.conversationsStarted || 0}</p>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="mt-3">
                    <p className="text-2xl font-bold">{aggregatedMetrics?.messagesSent || 0}</p>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="mt-3">
                    <p className="text-2xl font-bold">{aggregatedMetrics?.messagesReceived || 0}</p>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="mt-3">
                    <p className="text-2xl font-bold">{aggregatedMetrics?.avgResponseTime || 0}s</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className={`text-2xl font-bold ${(aggregatedMetrics?.autoClosureRate || 0) >= 70 ? 'text-green-500' : (aggregatedMetrics?.autoClosureRate || 0) >= 50 ? 'text-yellow-500' : 'text-red-500'}`}>
                    {aggregatedMetrics?.autoClosureRate || 0}%
                  </p>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className={`text-2xl font-bold ${(aggregatedMetrics?.escalationRate || 0) <= 20 ? 'text-green-500' : (aggregatedMetrics?.escalationRate || 0) <= 30 ? 'text-yellow-500' : 'text-red-500'}`}>
                    {aggregatedMetrics?.escalationRate || 0}%
                  </p>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className={`text-2xl font-bold ${(aggregatedMetrics?.errorRate || 0) <= 2 ? 'text-green-500' : (aggregatedMetrics?.errorRate || 0) <= 5 ? 'text-yellow-500' : 'text-red-500'}`}>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className={`text-2xl font-bold ${(aggregatedMetrics?.repeatedMessagesRate || 0) <= 10 ? 'text-green-500' : (aggregatedMetrics?.repeatedMessagesRate || 0) <= 15 ? 'text-yellow-500' : 'text-red-500'}`}>
                    {aggregatedMetrics?.repeatedMessagesRate || 0}%
                  </p>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className={`text-2xl font-bold ${(aggregatedMetrics?.sensitiveWordsCount || 0) <= 2 ? 'text-green-500' : (aggregatedMetrics?.sensitiveWordsCount || 0) <= 5 ? 'text-yellow-500' : 'text-red-500'}`}>
                    {aggregatedMetrics?.sensitiveWordsCount || 0}
                  </p>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold">{aggregatedMetrics?.avgTimeToClose || 0}min</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold">{aggregatedMetrics?.autoClosureRate || 0}%</p>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                    <div>
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className={`text-3xl font-bold mt-1 ${(aggregatedMetrics?.errorRate || 0) <= 2 ? 'text-green-500' : 'text-red-500'}`}>
                      {aggregatedMetrics?.errorRate || 0}%
                    </p>
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className={`text-3xl font-bold mt-1 ${(aggregatedMetrics?.sensitiveWordsCount || 0) === 0 ? 'text-green-500' : 'text-orange-500'}`}>
                      {aggregatedMetrics?.sensitiveWordsCount || 0}
                    </p>
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className={`text-3xl font-bold mt-1 ${(aggregatedMetrics?.repeatedMessagesRate || 0) <= 10 ? 'text-green-500' : 'text-red-500'}`}>
                      {aggregatedMetrics?.repeatedMessagesRate || 0}%
                    </p>
                    ...
                    <BarChart data={agentMetrics} layout="vertical">
                      ...
                      <Bar dataKey="healthScore" name="Score" radius={[0, 4, 4, 0]}>
                        {agentMetrics.map((entry, index) => (
                          <Cell key={index} fill={getHealthClassification(entry.healthScore).bgColor.replace('bg-', '#').replace('green-500', '22c55e').replace('blue-500', '3b82f6').replace('yellow-500', 'eab308').replace('red-500', 'ef4444')} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
