              ...
              <div className="p-5 sm:p-6">
                  ...
                  <div className="space-y-4">{fields.map((field, index) => {
                    ...
                    return <div key={field.id || index} className="rounded-lg border border-border/60 bg-background p-4">
                      ...
                      <div className="mt-4 space-y-4">
                         ...
                         <div className="flex items-center gap-2 rounded-lg border border-primary/15 bg-primary/5 px-3 py-2 text-xs text-muted-foreground"><CheckCircle2 className="h-4 w-4 shrink-0 text-primary" /><span>Ligação com o CRM configurada automaticamente.</span></div>
                ...
                {step === 2 && <div className="space-y-5">
                  <SectionHeading icon={Palette} title="Aparência" description="Personalize cores, marca e acabamento; a prévia muda enquanto você edita." />
                   ...
                   <div className="space-y-2"><Label>Arredondamento dos campos</Label><div className="flex items-center gap-3"><input aria-label="Arredondamento dos campos" type="range" min={0} max={24} value={cfg.radius ?? 10} onChange={(event) => setCfg("radius", Number(event.target.value))} className="h-2 flex-1 cursor-pointer accent-primary" /><Badge variant="outline" className="w-14 justify-center">{cfg.radius ?? 10}px</Badge></div></div>
