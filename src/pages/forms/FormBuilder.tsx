...
export default function FormBuilder() {
  ...
  return (
    <div className="min-h-screen bg-background">
              ...
              <div className="p-5 sm:p-6">
                ...
                {step === 5 && <div className="space-y-5">
                  ...
                  {isEdit && <div className="flex items-center justify-between rounded-lg border border-border/60 p-4"><div><p className="text-sm font-semibold">Formulário {form.status === "active" ? "ativo" : "inativo"}</p><p className="mt-1 text-xs text-muted-foreground">O controle também fica disponível no topo da página.</p></div><Switch checked={form.status === "active"} disabled={saving} onCheckedChange={(active) => save({ status: active ? "active" : "inactive" })} /></div>}
                   {isEdit && <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => navigator.clipboard.writeText(publicUrl).then(() => toast.success("Link copiado."))}><Copy /> Copiar link</Button><Button variant="outline" onClick={() => window.open(publicUrl, "_blank")}><ExternalLink /> Abrir formulário</Button><Button variant="outline" onClick={() => setEmbedOpen(true)}><Code2 /> Incorporar no site</Button></div>}
                </div>}
              </div>
              ...
              <div className="mb-3 flex items-center justify-between"><div><p className="text-sm font-semibold">Prévia ao vivo</p><p className="text-xs text-muted-foreground">{`/form/${effectiveSlug}`}</p></div><Badge variant="outline">{form.status === "active" ? "Ativo" : isEdit ? "Inativo" : "Rascunho"}</Badge></div>
                <div className="overflow-hidden rounded-lg p-4" style={{ background: cfg.backgroundColor || "#f6f7f9" }}><div className="p-4 shadow-sm" style={{ borderRadius: (cfg.radius ?? 10) + 6, background: "#ffffff", border: "1px solid #ececf0" }}>{cfg.coverUrl && <img src={normalizeLink(cfg.coverUrl)} alt="Capa" className="mb-4 h-24 w-full object-cover" style={{ borderRadius: cfg.radius ?? 10 }} />}{cfg.logoUrl && <div className="mb-4 flex h-14 w-48 items-center" style={{ marginInline: cfg.align === "center" ? "auto" : undefined }}><img src={normalizeLink(cfg.logoUrl)} alt="Logo" className="h-full w-full object-contain object-left" style={{ objectPosition: cfg.align === "center" ? "center" : "left center" }} /></div>}<div style={{ textAlign: cfg.align || "left" }}><p className="text-base font-semibold" style={{ color: cfg.textColor || "#18181b" }}>{previewTitle}</p><p className="mt-1 text-xs opacity-70" style={{ color: cfg.textColor || "#18181b" }}>{previewDescription}</p></div>
                 ...
                 <div className="mt-4 space-y-3">{fields.filter((field) => field.is_active !== false && field.label.trim() && pageOf(field) === currentPage).slice(0, 6).map((field, index) => {
                   ...
                   const box = { borderRadius: cfg.radius ?? 10 };
                   const fieldText = cfg.textColor || "#18181b";
                   const muted = "rgba(24, 24, 27, 0.55)";
                   return <div key={field.id || index}>
                     <p className="mb-1 flex items-center gap-1.5 text-[11px] font-medium" style={{ color: fieldText }}><item.icon className="h-3 w-3" />{field.label}{field.required && " *"}</p>
                     {field.field_type === "radio" ? <div className="space-y-1.5">{(options.length ? options : ["Opção 1", "Opção 2"]).slice(0, 4).map((option: string, optionIndex: number) => <div key={`${option}-${optionIndex}`} className="flex items-center gap-2 text-[10px]" style={{ color: muted }}><span className="h-3 w-3 rounded-full border" style={{ borderColor: "#d4d4da" }} />{option}</div>)}</div>
                       : field.field_type === "select" ? <div className="flex h-9 items-center justify-between border px-3 text-[10px]" style={{ ...box, borderColor: "#dcdce2", background: "#ffffff", color: muted }}><span>{options[0] || field.placeholder || "Selecione uma opção"}</span><ChevronDown className="h-3 w-3" /></div>
                       : field.field_type === "checkbox" ? <div className="flex items-center gap-2 text-[10px]" style={{ color: muted }}><span className="h-3.5 w-3.5 rounded-[4px] border" style={{ borderColor: "#d4d4da" }} />{field.placeholder || "Confirmo que li e aceito"}</div>
                        : field.field_type === "file" ? <div className="flex items-center gap-2 border border-dashed px-3 py-3 text-[10px]" style={{ ...box, borderColor: "#c9c9d1", background: "#ffffff", color: muted }}><FileText className="h-4 w-4 shrink-0" style={{ color: cfg.primaryColor || "#3daa57" }} />Arraste um arquivo ou clique para enviar imagens ou PDF (até 8 MB)</div>
                       : field.field_type === "textarea" ? <div className="flex h-16 items-start border px-3 py-2 text-[10px]" style={{ ...box, borderColor: "#dcdce2", background: "#ffffff", color: muted }}>{field.placeholder || "Digite sua resposta"}</div>
                       : <div className="flex h-9 items-center border px-3 text-[10px]" style={{ ...box, borderColor: "#dcdce2", background: "#ffffff", color: muted }}>{field.placeholder || "Digite sua resposta"}</div>}
                   </div>;
                 })}</div>
                 <div className="mt-4 flex h-10 items-center justify-center text-xs font-semibold text-white" style={{ background: cfg.buttonColor || "#3daa57", borderRadius: cfg.radius ?? 10 }}>{currentPage < totalPages ? "Continuar" : previewButton}</div></div></div>
            </Card>
          </div>
        </div>
      </main>
      ...
      <FormEmbedDialog open={embedOpen} onOpenChange={setEmbedOpen} formName={form.name || "Formulário"} slug={effectiveSlug} />
    </div>
  );
}
