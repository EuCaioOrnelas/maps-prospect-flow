import { DollarSign, TrendingUp, AlertTriangle, BarChart3, Info, TrendingDown, ArrowUpRight, ArrowDownRight } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { useRevenueDashboardStats, useRevenueSettings, useRevenueLeads } from "@/hooks/useRevenueData";
...
const RevenueInsights = () => {
  ...
  // Receita em Risco: leads hot/very_hot com risk_state != OK
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Estimativas baseadas no engajamento, ticket médio e taxas de conversão configuradas
        </p>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-muted-foreground">
              ...
              <p className="text-xs leading-relaxed">
                <strong>Receita Potencial:</strong> (Engajados + Quentes) × ticket médio — cenário ideal.
                <br />
                <strong>Receita Esperada:</strong> Σ (leads por nível × taxa de conversão × ticket) — estimativa realista.
                <br />
                <strong>Receita em Risco:</strong> Valor dos leads engajados/quentes que estão esfriando ou em risco — o que você pode perder.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card className="bg-card border-border/50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <DollarSign size={14} /> Receita Potencial
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-card border-border/50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <TrendingUp size={14} /> Receita Esperada
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-foreground">{fmt(receitaEsperada)}</p>
            <p className="text-xs text-muted-foreground mt-1">Ponderada pelas taxas de conversão</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border/50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <AlertTriangle size={14} /> Receita em Risco
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-destructive">{fmt(receitaEmRisco)}</p>
            <p className="text-xs text-muted-foreground mt-1">
              {atRiskLeads.length} leads engajados/quentes em risco
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-card border-border/50">
        <CardHeader>
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            <BarChart3 size={16} /> Detalhamento por Nível
          </CardTitle>
          <CardDescription>Quantidade, taxa de conversão e receita esperada por nível</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {(["VERY_HOT", "HOT", "ENGAGED", "COLD"] as const).map((bucket) => {
              ...
              const colors: Record<string, string> = {
                COLD: "bg-blue-500", ENGAGED: "bg-yellow-500", HOT: "bg-orange-500", VERY_HOT: "bg-red-500",
              };
              ...
              return (
                <div key={bucket} className="space-y-1.5">
                  ...
                  <div className="h-2.5 bg-secondary rounded-full overflow-hidden">
                    <div className={cn("h-full rounded-full transition-all", colors[bucket])} style={{ width: `${width}%` }} />
                  </div>
                </div>
              );
            })}
      ...
      <Card className="bg-card border-border/50">
        <CardHeader>
          <CardTitle className="text-sm font-medium text-muted-foreground">Parâmetros usados</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
            ...
            <div>
              <p className="text-muted-foreground text-xs">Taxa Morno</p>
          ...
          <p className="text-[10px] text-muted-foreground/60 mt-3">
            Ajuste em Configurações para refletir a realidade do seu negócio.
          </p>
        </CardContent>
      </Card>
    </div>
