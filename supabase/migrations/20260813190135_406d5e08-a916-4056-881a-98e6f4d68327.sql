...
DECLARE
  ...
  -- Explicit code typed by the user (trial / checkout / signup trigger) wins over links.
  v_is_manual_code boolean := (p_source = 'referral_code')
    OR (v_referral_code IS NOT NULL AND p_click_id IS NULL AND p_partner_id IS NULL);
BEGIN
  ...
  IF v_existing.id IS NULL THEN
    ...
    RETURN jsonb_build_object('ok', true, 'status', 'created', 'lead_id', v_lead_id,
                              ...
                              'attribution_source', CASE WHEN v_is_manual_code THEN 'referral_code' ELSE 'referral_link' END);
  END IF;
