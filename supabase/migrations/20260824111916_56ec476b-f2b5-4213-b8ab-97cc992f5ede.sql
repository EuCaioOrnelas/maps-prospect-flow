ALTER TABLE public.partner_settings
  ADD COLUMN IF NOT EXISTS first_month_boost_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS first_month_boost_percent numeric NOT NULL DEFAULT 50,
  ADD COLUMN IF NOT EXISTS first_month_boost_until timestamptz;
...
BEGIN
  ...
  IF NEW.customer_user_id IS NOT NULL THEN
    SELECT MIN(paid_at), COUNT(*) INTO v_first_paid_at, v_prior_sales
    FROM partner_sales
    ...
    v_is_first := (v_prior_sales = 0);
  END IF;

  v_percent := public.get_partner_commission_percent(NEW.partner_id);

  -- Bônus temporário: 50% (configurável) na primeira mensalidade do cliente indicado
  IF v_is_first
     ...
     AND COALESCE(v_settings.first_month_boost_percent, 0) > v_percent
  THEN
