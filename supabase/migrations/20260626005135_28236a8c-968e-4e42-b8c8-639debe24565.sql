...
BEGIN
  ...
  IF v_cur = 'USD' OR v_cur = 'US$' OR v_cur = '$' THEN
  ...
  -- Moeda desconhecida: assume BRL para não inflar o custo
  RETURN p_amount;
END;
$$;
...
BEGIN
  ...
  -- Aggregate outbound messages linked to this campaign, normalizing every billing_amount to BRL
  SELECT
    COUNT(*),
...
-- Template average cost — also normalized to BRL
...
BEGIN
  -- 1) Same template, same owner, last 90d
  ...
  -- 2) Same template globally, last 30d
  SELECT AVG(public.meta_fx_to_brl(billing_amount, billing_currency))
  INTO v_avg
  FROM public.chat_messages
  WHERE direction = 'outbound'
  ...
  IF v_cat <> '' THEN
    -- 3) Same category for owner, last 90d
    SELECT AVG(public.meta_fx_to_brl(billing_amount, billing_currency))
    INTO v_avg
    FROM public.chat_messages
    WHERE direction = 'outbound'
    ...
    -- 4) Same category globally, last 30d
    SELECT AVG(public.meta_fx_to_brl(billing_amount, billing_currency))
    INTO v_avg
    FROM public.chat_messages
    WHERE direction = 'outbound'
