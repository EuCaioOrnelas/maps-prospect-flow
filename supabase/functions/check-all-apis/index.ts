...
async function checkResend(): Promise<ApiResult> {
  ...
  try {
    const [domainsRes, accountRes] = await Promise.all([
      fetch('https://api.resend.com/domains', { headers: { Authorization: `Bearer ${key}` } }),
      fetch('https://api.resend.com/account', { headers: { Authorization: `Bearer ${key}` } }).catch(() => null),
    ]);
    ...
    if (accountRes && accountRes.ok) account = await accountRes.json();
    return { service: 'Resend', category: 'email', status: 'ok', message: `${(d.data || []).length} domínio(s) configurado(s): ${domainNames}`, details: { domains, account } };
  } catch (e) {
