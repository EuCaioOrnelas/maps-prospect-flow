...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "create") {
      const { partner_id, slug, label, description, utm_source, utm_medium, utm_campaign } = body;
    ...
    if (action === "update") {
      const { id, label, description, is_active, utm_source, utm_medium, utm_campaign } = body;
      const patch: any = {};
