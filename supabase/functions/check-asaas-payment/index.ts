...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Checking payment status", { pixId });

    // pixId is actually the Asaas payment ID
    const paymentRes = await fetch(`${ASAAS_API}/payments/${pixId}`, {
      headers: {
        "access_token": apiKey,
    ...
    if (!paymentRes.ok) {
      throw new Error(`Payment check failed: ${JSON.stringify(paymentJson)}`);
    }

    const status = paymentJson.status;
    logStep("Payment status", { pixId, status });

    // Map Asaas statuses to our expected format
    ...
    if (mappedStatus === "PAID") {
      const subscriptionId = paymentJson.subscription;
      ...
      if (subscriptionId) {
        ...
        if (leads && leads.length > 0) {
          ...
          const profileId = lead.user_id || null;
          ...
          logStep("Checkout completed", { subscriptionId });
        }
      }
    }
