  ...
  try {
    ...
    if (action === "preview" || action === "send_test") {
      ...
      if (action === "preview") {
        return json({
          subject,
          html: lifecycleLayout({ body: compiledBody, preheader, unsubscribeUrl: "#", dayOffset: step.day_offset }),
        });
      }
      ...
      const unsubscribeUrl = await buildUnsubscribeUrl(supabaseUrl, delivery.id, trackingSecret);
       const html = lifecycleLayout({ body: tracked, preheader, unsubscribeUrl, isTest: true, dayOffset: step.day_offset });
