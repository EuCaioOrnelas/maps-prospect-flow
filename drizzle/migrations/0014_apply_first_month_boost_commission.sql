...
BEGIN
  ...
  IF NEW.customer_user_id IS NOT NULL THEN
    ...
    -- Primeira comissão deste cliente para este parceiro?
    SELECT NOT EXISTS (
  ...
  v_base_percent := public.get_partner_commission_percent(NEW.partner_id);
  v_percent := v_base_percent;

  -- Boost de primeiro mês (ex.: 50%) aplicado somente na primeira comissão do cliente
  IF v_is_first
     AND v_partner.custom_commission_percent IS NULL
     AND COALESCE(v_settings.first_month_boost_enabled, false)
     AND COALESCE(v_settings.first_month_boost_percent, 0) > v_base_percent
     AND (v_settings.first_month_boost_until IS NULL OR NEW.paid_at <= v_settings.first_month_boost_until)
  THEN
    v_percent := v_settings.first_month_boost_percent;
  END IF;

  v_commission_cents := (NEW.amount_cents * v_percent / 100)::BIGINT;

  IF NEW.customer_user_id IS NOT NULL THEN
    SELECT COALESCE(p.extra_numbers, 0) AS numbers,
           COALESCE(p.extra_contacts_packs, 0) AS contacts,
           COALESCE(p.extra_opportunities_packs, 0) AS opportunities
      INTO v_profile
    FROM profiles p WHERE p.id = NEW.customer_user_id;
